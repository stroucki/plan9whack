...
fn whackmatch(
    ...
    loop {
                        ...
                        == src[candidate_match_position + bestlen])
        {
            ...
            {
                unsafe {
                    let a = ptr::read_unaligned(src.as_ptr().add(current_match_position) as *const u64);
                    let b = ptr::read_unaligned(src.as_ptr().add(candidate_match_position) as *const u64);
                    if a == b {
